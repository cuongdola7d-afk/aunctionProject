...
public class RealtimeClientHandler implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            LOGGER.warn("Client ngắt kết nối: {}", e.getMessage());
        } finally {
            if (client != null) {
    ...
    private void handleMessage(ClientConnection client, String line) {
        try {
            ...
            LOGGER.info("Nhận message type: {} từ client: {}", message.getType(), client.getConnectionId());
            ...
            LOGGER.warn("Lỗi xử lý message: {}", e.getMessage(), e);
    ...
    private void handleSubscribe(ClientConnection client, SocketMessage message) {
        ...
        LOGGER.info("Client {} đã subscribe auction: {}", client.getConnectionId(), auctionId);
    }
    ...
    private void handlePlaceBid(ClientConnection client, SocketMessage message) {
        ...
        LOGGER.info("Bid thành công: auction={} bidder={} amount={}", auction.getId(), bidder.getUsername(),
                request.getAmount());
    }
    ...
    private void sendError(ClientConnection client, String errorMessage) {
        try {
            ...
            LOGGER.warn("Không gửi được error cho client: {}", e.getMessage());
        }
    }
    ...
    private void sendAuctionEvent(ClientConnection client, Object payload) {
        try {
            ...
            LOGGER.warn("Không gửi được event cho client: {}", e.getMessage());
        }
    }
